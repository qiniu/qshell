...
func rsDemo(bucket, key, bucketSrc, keySrc, bucketDest, keyDest string) {
	...
	if err != nil {
		// 产生错误
		fmt.Println("rs.Stat failed:", err)
		return
	}
	// 处理返回值
	fmt.Println(ret)
	...
	if err != nil {
		// 产生错误
		fmt.Println("rs.Copy failed:", err)
		return
	}
	// @endgist
...
func batchDemo(bucket, key, bucket1, key1, bucket2, key2, bucket3, key3, bucket4, key4 string) {
	...
	if err != nil {
		// 产生错误
		fmt.Println("rs.BatchStat failed:", err)
		return
	}
	// 处理返回值
	...
	if err != nil {
		// 产生错误
		fmt.Println("rs.BatchCopy failed:", err)
		return
	}
	...
	if err != nil {
		// 产生错误
		fmt.Println("rs.BatchMove failed:", err)
		return
	}
	...
	if err != nil {
		// 产生错误
		fmt.Println("rs.BatchDelete failed:", err)
		return
	}
	...
	if err != nil {
		// 产生错误
		fmt.Println("rs.Batch failed:", err)
		return
	}
	for _, ret := range *rets {
		fmt.Println(ret.Code, ret.Error)
	}
	// @endgist
}
