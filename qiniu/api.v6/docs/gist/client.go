...
import (
	"fmt"
	"github.com/tonycai653/iqshell/qiniu/api.v6/io"
...
func uploadFileDemo(localFile, key, uptoken string) {
	...
	if err != nil {
		//上传产生错误
		fmt.Print("io.PutFile failed:", err)
		return
	}

	//上传成功，处理返回值
	fmt.Print(ret.Hash, ret.Key)
	// @endgist
}
...
func resumableUploadFileDemo(localFile, key, uptoken string) {
	...
	if err != nil {
		//上传产生错误
		fmt.Print("resumable.io.Put failed:", err)
		return
	}
