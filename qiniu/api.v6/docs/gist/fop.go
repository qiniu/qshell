...
import (
	"fmt"
	"github.com/tonycai653/iqshell/qiniu/api.v6/fop"
)
...
func getImageInfo(imageUrl string) {
	...
	if err != nil {
		// 产生错误
		fmt.Println("fop getImageInfo failed:", err)
		return
	}
	fmt.Println(infoRet.Height, infoRet.Width, infoRet.ColorModel,
		infoRet.Format)
	// @endgist
}
...
func getExif(imageUrl string) {
	...
	if err != nil {
		// 产生错误
		fmt.Println("fop getExif failed:", err)
		return
	}

	// 处理返回结果
	for _, item := range exifRet {
		fmt.Println(item.Type, item.Val)
	}
	// @endgist
}
