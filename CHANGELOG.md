...
4. `qshell sandbox template` 支持按 `name` 自动定位模板：未提供 `template_id` 时，`build` 会先按 name 查找远端模板，命中则 rebuild，未命中再创建；`get` / `publish` / `delete` / `unpublish` 未传显式 ID 时，也可从 `qshell.sandbox.toml` 的 `name` 定位模板。若希望同一份配置跨环境复用，请只保留 `name`，不要提交自动回写的 `template_id`
...
## 更新
1. stat 命令可展示文件的 md5 和文件的禁用状态

## 废除
1. 废弃 listbucket2 的 --api-version，使用此选项会被无视，内部强制使用 list v1 接口
