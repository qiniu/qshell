...
func List(info ListInfo) {
	...
	objects, err := bucket.List(apiInfo)
	if err != nil {
		log.Error(err)
		return
	}
...
func filteredByTime(startDate, endDate time.Time, item bucket.ListItem) bool {
...
func filteredBySuffix(suffixes []string, item bucket.ListItem) bool {
	if len(suffixes) == 0 {
		return false
	}
