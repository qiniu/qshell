...
func AllBuckets(info ListApiInfo) (buckets []BucketInfo, err error) {
	return allBuckets(workspace.GetConfig(), info)
}

func allBuckets(cfg *config.Config, info ListApiInfo) (buckets []BucketInfo, err error) {
	...
	err = bucketManager.Client.CredentialedCall(workspace.GetContext(), bucketManager.Mac, auth.TokenQiniu, &buckets, "POST", reqURL, nil)
	return
}
