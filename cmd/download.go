...
have already in local disk and need to skip download or not.`,
	...
	cmd.Flags().Int64VarP(&info.DownloadCfg.SliceSize, "slice-size", "", 4*1024*1024, "slice size; when using slice download, the size of each slice; unit:B")
	cmd.Flags().IntVarP(&info.DownloadCfg.SliceConcurrentCount, "slice-concurrent-count", "", 10, "concurrency of slice downloads")
	cmd.Flags().Int64VarP(&info.DownloadCfg.SliceFileSizeThreshold, "slice-file-size-threshold", "", 40*1024*1024, "file threshold for downloading slices. When slice downloading is enabled and the file size is greater than this threshold, slice downloading will be enabled; unit:B")
